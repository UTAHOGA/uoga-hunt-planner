...
const APP_BUILD = 'build-2026-03-21-28';
...
function getDwrBoundaryUrl(hunt = null) {
...
function buildOfficialHuntCardHtml(hunt, remoteInfo = null, options = {}) {
  const title = escapeHtml(getHuntTitle(hunt) || getHuntCode(hunt));
  const unit = escapeHtml(getUnitName(hunt) || getUnitValue(hunt) || 'N/A');
  const huntCode = escapeHtml(getHuntCode(hunt) || 'N/A');
  ...
  const huntType = escapeHtml(getHuntTypeLabel(getHuntType(hunt)) || 'N/A');
  const seasonText = escapeHtml(getEffectiveSeasonText(hunt, remoteInfo) || 'N/A');
  const boundaryName = escapeHtml((remoteInfo?.boundaryNames || []).join(' | ') || getUnitName(hunt) || 'N/A');
  const kicker = options.kicker || 'Utah DWR-Style Hunt Info';
  const note = options.note || 'Official season and boundary details can be refreshed separately from the boundary geometry.';

  return `
    <div class="hunt-popup-card">
      <div class="hunt-popup-kicker">${escapeHtml(kicker)}</div>
      <div class="hunt-official-titlebar">
        <h4>${title}</h4>
      </div>
      <div class="hunt-official-subtitle">${unit}</div>
      <div class="hunt-popup-grid">
        <div><strong>Hunt Number:</strong><br>${huntCode}</div>
      ...
      <div class="hunt-official-block">${seasonText}</div>
      <div class="hunt-official-bar">Boundary</div>
      <div class="hunt-official-block">${boundaryName}</div>
      <div class="hunt-official-actions" style="margin-top:10px;">
        <button type="button" class="btn-primary" data-action="print-hunt" data-hunt-code="${huntCode}">Print</button>
        <button type="button" class="btn-secondary" data-action="download-kml" data-hunt-code="${huntCode}">KML for Google Earth</button>
      </div>
      <div class="hunt-popup-note">${escapeHtml(note)}</div>
    </div>
  `;
}
...
function coordinatesToKml(coordinates) {
  ...
  if (coordinates.length && Array.isArray(coordinates[0]) && typeof coordinates[0][0] === 'number') {
    return coordinates
      .map(pair => `${Number(pair[0])},${Number(pair[1])},0`)
      .join(' ');
  }
  return coordinates.map(child => coordinatesToKml(child)).filter(Boolean).join('');
}
...
function geometryToKml(geometry) {
  ...
  if (geometry.type === 'Polygon' && Array.isArray(geometry.coordinates)) {
    ...
    return `
      <Polygon>
        <outerBoundaryIs><LinearRing><coordinates>${coordinatesToKml(outer)}</coordinates></LinearRing></outerBoundaryIs>
        ${inners.map(ring => `<innerBoundaryIs><LinearRing><coordinates>${coordinatesToKml(ring)}</coordinates></LinearRing></innerBoundaryIs>`).join('')}
      </Polygon>
    `;
  }
  if (geometry.type === 'MultiPolygon' && Array.isArray(geometry.coordinates)) {
    return `
      <MultiGeometry>
        ${geometry.coordinates.map(poly => geometryToKml({ type: 'Polygon', coordinates: poly })).join('')}
      </MultiGeometry>
    `;
  }
  if (Array.isArray(geometry.rings)) {
    return geometryToKml({ type: 'Polygon', coordinates: geometry.rings });
  }
  return '';
}

function downloadSelectedHuntKml(hunt = selectedHunt) {
  if (!hunt) return;
  const features = getSelectedBoundaryFeaturesForHunt(hunt);
  if (!features.length) {
    alert('No boundary geometry is currently available for this hunt.');
    return;
  }

  const placemarks = features.map((feature, index) => {
    const geometry = feature?.geometry || null;
    const name = getBoundaryFeatureName(feature) || `${getUnitName(hunt) || getHuntCode(hunt)} ${index + 1}`;
    return `
      <Placemark>
...
<kml xmlns="http://www.opengis.net/kml/2.2">
  <Document>
    <name>${escapeHtml(getHuntTitle(hunt) || getHuntCode(hunt))}</name>
    ${placemarks}
  </Document>
</kml>`;
  ...
  a.href = url;
  a.download = `${safe(getHuntCode(hunt) || 'selected-hunt').replace(/[^a-z0-9_-]+/gi, '_')}.kml`;
...
function printSelectedHunt(hunt = selectedHunt, remoteInfo = null) {
  if (!hunt) return;
  const printWindow = window.open('', '_blank', 'width=980,height=900');
  if (!printWindow) return;

  const title = getHuntTitle(hunt) || getHuntCode(hunt);
  const seasonText = getEffectiveSeasonText(hunt, remoteInfo);
  const boundaryName = (remoteInfo?.boundaryNames || []).join(' | ') || getUnitName(hunt) || getUnitValue(hunt) || 'N/A';
  ...
  <html>
    <head>
      ...
      <style>
        body { font-family: Georgia, "Times New Roman", serif; color: #2f2118; margin: 28px; }
        h1 { margin: 0 0 8px; font-size: 28px; }
        .meta { margin-bottom: 18px; color: #5f4a39; }
        .grid { display: grid; grid-template-columns: repeat(2, minmax(220px, 1fr)); gap: 10px 18px; margin-bottom: 18px; }
        .section { margin-top: 18px; padding: 8px 10px; background: #ecefe2; font-weight: 700; }
        .note { margin-top: 12px; color: #5f4a39; line-height: 1.45; }
      </style>
    </head>
    <body>
      <h1>${escapeHtml(title)}</h1>
      <div class="meta">U.O.G.A. Hunt Planner print view</div>
      <div class="grid">
        ...
        <div><strong>Species:</strong><br>${escapeHtml(getSpeciesDisplay(hunt) || 'N/A')}</div>
        <div><strong>Sex:</strong><br>${escapeHtml(getNormalizedSex(hunt) || 'N/A')}</div>
        <div><strong>Weapon:</strong><br>${escapeHtml(getWeapon(hunt) || 'N/A')}</div>
        <div><strong>Hunt Type:</strong><br>${escapeHtml(getHuntTypeLabel(getHuntType(hunt)) || 'N/A')}</div>
        <div><strong>Season:</strong><br>${escapeHtml(seasonText || 'N/A')}</div>
        <div><strong>Boundary:</strong><br>${escapeHtml(boundaryName)}</div>
      </div>
      <div class="section">Official Hunt Summary</div>
      <div class="note">Printed from the U.O.G.A. Hunt Planner. Use the Utah DWR Official section on the website when you want the live state reference alongside this summary.</div>
...
async function showOfficialHuntPopup(hunt, latlng = null) {
  if (!hunt || !map) return;

  const popupLatLng = latlng || (() => {
    const center = getSelectedBoundaryCenter(hunt) || getCesiumSelectedCenter(hunt);
    return center ? L.latLng(center.lat, center.lng) : null;
  })();

  if (!popupLatLng) return;

  const loadingHtml = buildOfficialHuntCardHtml(hunt, null, {
    kicker: 'Utah DWR-Style Hunt Info',
    note: 'Loading latest official season details from Utah DWR...'
  });

  const popup = L.popup({ maxWidth: 340, className: 'hunt-info-popup' })
  ...
  try {
    const remoteInfo = await fetchRemoteHuntInfo(hunt);
    popup.setContent(buildOfficialHuntCardHtml(hunt, remoteInfo, {
      kicker: 'Utah DWR-Style Hunt Info',
      note: 'This popup uses your selected hunt plus the current DWR mobile hunt table.'
    }));
    if (selectedHunt && getHuntCode(selectedHunt) === getHuntCode(hunt)) {
      renderAreaInfo(remoteInfo);
    }
  } catch (error) {
    popup.setContent(buildOfficialHuntCardHtml(hunt, null, {
      kicker: 'Utah DWR-Style Hunt Info',
      note: 'Live DWR season details were unavailable just now, so this popup is showing planner data only.'
    }));
  }
}
...
function updateDwrBoundaryEmbed(hunt = null, options = {}) {
      ...
      dwrBoundarySnapshot.innerHTML = `
        <div class="dwr-snapshot-card">
          ...
          <div class="dwr-snapshot-actions">
            <button type="button" class="btn-primary" data-action="load-dwr-map-here">Load Full Utah DWR Planner Here</button>
            <a href="${escapeHtml(src)}" target="_blank" rel="noopener noreferrer">
              <button type="button" class="btn-secondary">Open Utah DWR in New Tab</button>
            </a>
          </div>
        </div>
      `;
  ...
  if (dwrBoundaryMeta) {
    dwrBoundaryMeta.textContent = hunt
      ? `Showing an official Utah DWR snapshot for ${getUnitName(hunt) || getHuntTitle(hunt)}. Load the full DWR planner here only if you need more detail.`
      : 'Showing the Utah DWR hunt planner. Select a hunt, then use the boundary button to load that official map here.';
  }
...
function attachHuntResultsInteraction(container) {
  ...
  container.addEventListener('click', e => {
    ...
    if (action === 'print-hunt') {
      const code = actionEl.getAttribute('data-hunt-code');
      const hunt = huntData.find(h => getHuntCode(h) === code) || selectedHunt;
      if (hunt) printSelectedHunt(hunt);
    }
    if (action === 'download-kml') {
      const code = actionEl.getAttribute('data-hunt-code');
      const hunt = huntData.find(h => getHuntCode(h) === code) || selectedHunt;
      if (hunt) downloadSelectedHuntKml(hunt);
    }
    if (action === 'zoom-selected-hunt') {
      const code = actionEl.getAttribute('data-hunt-code');
      const hunt = huntData.find(h => getHuntCode(h) === code) || selectedHunt;
      if (hunt) zoomToHuntSelection(hunt);
    }
      return;
    }
    ...
    { type: 'FeatureCollection', features },
    {
      ...
      onEachFeature: (feature, layer) => {
        layer.on('click', evt => {
          ...
          showOfficialHuntPopup(matchedHunt, evt.latlng);
        });
      ...
      <div class="hunt-official-shell">
        ...
        <div class="hunt-official-side">
          <div class="hunt-official-actions">
            <button type="button" class="btn-primary" data-action="zoom-selected-hunt" data-hunt-code="${huntCode}">Zoom To</button>
            <button type="button" class="btn-primary" data-action="print-hunt" data-hunt-code="${huntCode}">Print</button>
            <button type="button" class="btn-secondary" data-action="download-kml" data-hunt-code="${huntCode}">KML for Google Earth</button>
            <button type="button" class="btn-secondary" data-action="load-dwr-map-here" data-hunt-code="${huntCode}">Utah DWR Official</button>
          </div>
        </div>
      </div>
    </div>
