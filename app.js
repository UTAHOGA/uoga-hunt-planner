...
function getFilteredHunts() {
  ...
  return huntData.filter(h => {
    ...
    const sexOk = matchesFilter(sex, getNormalizedSex(h));
...
function renderAreaInfo() {
  ...
  setHtml([areaInfoEl, areaInfoMobileEl], `
    ...
    <strong>Species:</strong> ${escapeHtml(getSpeciesDisplay(selectedHunt))}<br>
    <strong>Sex:</strong> ${escapeHtml(getNormalizedSex(selectedHunt))}<br>
