...
const APP_BUILD = 'build-2026-03-21-32';
...
function buildOfficialHuntCardHtml(hunt, remoteInfo = null, options = {}) {
  ...
  const kicker = options.kicker || 'U.O.G.A. Hunt Planner Info';
...
function printSelectedHunt(hunt = selectedHunt, remoteInfo = null) {
  ...
  <html>
    ...
    <body>
      ...
      <div class="note">Printed from the U.O.G.A. Hunt Planner. Use the official hunt reference section on the website when you want the live state reference alongside this summary.</div>
...
async function showOfficialHuntPopup(hunt, latlng = null) {
  ...
  const loadingHtml = buildOfficialHuntCardHtml(hunt, null, {
    kicker: 'U.O.G.A. Hunt Planner Info',
    note: 'Loading latest official season details from the state hunt table...'
  });
  ...
  try {
    ...
    popup.setContent(buildOfficialHuntCardHtml(hunt, remoteInfo, {
      kicker: 'U.O.G.A. Hunt Planner Info',
      note: 'This popup uses your selected hunt plus the current official state hunt table.'
    }));
...
function updateDwrBoundaryEmbed(hunt = null, options = {}) {
      ...
      dwrBoundarySnapshot.innerHTML = `
        <div class="dwr-snapshot-card">
          ...
          <div class="dwr-snapshot-actions">
            <button type="button" class="btn-primary" data-action="load-dwr-map-here">Load Full Official Hunt Reference Here</button>
            <a href="${escapeHtml(src)}" target="_blank" rel="noopener noreferrer">
              <button type="button" class="btn-secondary">Open Official Hunt Reference in New Tab</button>
            </a>
          </div>
        </div>
      `;
  ...
  if (dwrBoundaryMeta) {
    dwrBoundaryMeta.textContent = hunt
      ? `Showing an official hunt reference snapshot for ${getUnitName(hunt) || getHuntTitle(hunt)}. Load the full official reference here only if you need more detail.`
      : 'Showing the official state hunt reference. Select a hunt, then use the hunt info button to load that reference here.';
  }
      ...
      <div class="hunt-official-shell">
        ...
        <div class="hunt-official-side">
          <div class="hunt-official-actions">
            ...
            <button type="button" class="btn-secondary" data-action="load-dwr-map-here" data-hunt-code="${huntCode}">Official Hunt Reference</button>
          </div>
        </div>
      </div>
    </div>
