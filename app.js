...
function updateGoogleMapsEmbed(hunt = null) {
  ...
  let src = `https://www.google.com/maps/embed/v1/search?key=${encodeURIComponent(GOOGLE_EMBED_API_KEY)}&q=${encodeURIComponent('Utah')}`;
  ...
  if (hunt) {
    ...
    const centerLat = hasValidHuntCenter ? lat : hasValidTrustedCenter ? trustedCenter[0] : null;
    const centerLng = hasValidHuntCenter ? lng : hasValidTrustedCenter ? trustedCenter[1] : null;

    if (Number.isFinite(centerLat) && Number.isFinite(centerLng)) {
      const center = `${centerLat},${centerLng}`;
      src = `https://www.google.com/maps/embed/v1/view?key=${encodeURIComponent(GOOGLE_EMBED_API_KEY)}&center=${encodeURIComponent(center)}&zoom=9&maptype=roadmap`;
    } else {
      const query = [getUnitName(hunt), 'hunt unit', 'Utah'].filter(Boolean).join(' ');
      src = `https://www.google.com/maps/embed/v1/search?key=${encodeURIComponent(GOOGLE_EMBED_API_KEY)}&q=${encodeURIComponent(query)}`;
    }
  }
...
function updateDwrBoundaryEmbed(hunt = null, options = {}) {
  ...
  if (dwrBoundaryMeta) {
    dwrBoundaryMeta.textContent = hunt
      ? `Showing the official Utah DWR hunt planner for ${getUnitName(hunt) || getHuntTitle(hunt)}.`
      : 'Showing the Utah DWR hunt planner. Select a hunt, then use the boundary button to load that official map here.';
  }

  if (options.scrollIntoView) {
    dwrBoundaryEmbed.scrollIntoView({ behavior: 'smooth', block: 'start' });
  }
}
